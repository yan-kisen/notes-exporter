on run argv
	...
	set envExcludeAccounts to item 9 of argv
	set envIncludeAccounts to item 10 of argv
	set envExcludeFolders to item 11 of argv
	set envIncludeFolders to item 12 of argv
	...
	-- Parse filter lists
	set excludeAccounts to {}
	if envExcludeAccounts ≠ "" then
		set excludeAccounts to my splitByDelimiter(envExcludeAccounts, ",")
	end if

	log "[excludeAccounts]: " & excludeAccounts

	set includeAccounts to {}
	if envIncludeAccounts ≠ "" then
		set includeAccounts to my splitByDelimiter(envIncludeAccounts, ",")
	end if

	log "[includeAccounts]: " & includeAccounts

	set excludeFolders to {}
	if envExcludeFolders ≠ "" then
		set excludeFolders to my splitByDelimiter(envExcludeFolders, ",")
	end if

	log "[excludeFolders]: " & excludeFolders

	set includeFolders to {}
	if envIncludeFolders ≠ "" then
		set includeFolders to my splitByDelimiter(envIncludeFolders, ",")
	end if

	log "[includeFolders]: " & includeFolders
...
--			set accountName to my makeValidFilename(originalAccountName)
			...
			if shouldProcessAccount then
				...
				log "Skipping account: " & originalAccountName & " (filtered out)"
			end if
		end repeat

		repeat with anAccount in filteredAccounts
...
--				set folderName to my makeValidFilename(originalFolderName)
				...
				if (count of includeFolders) > 0 then
				    repeat with includeItem in includeFolders
				        set lowerOriginal to do shell script "echo " & quoted form of originalFolderName & " | tr '[:upper:]' '[:lower:]'"
				        set lowerInclude to do shell script "echo " & quoted form of includeItem & " | tr '[:upper:]' '[:lower:]'"
				        if lowerOriginal = lowerInclude then
				    ...
				    set shouldProcessFolder to true
				    repeat with excludeItem in excludeFolders
				        set lowerOriginal to do shell script "echo " & quoted form of originalFolderName & " | tr '[:upper:]' '[:lower:]'"
				        set lowerExclude to do shell script "echo " & quoted form of excludeItem & " | tr '[:upper:]' '[:lower:]'"
				        if lowerOriginal = lowerExclude then
				            set shouldProcessFolder to false
				...
				if shouldProcessFolder then
					set end of filteredFolders to aFolder
				else
					log "Skipping folder: " & originalFolderName & " in account " & originalAccountName & " (filtered out)"
				end if
			end repeat
			repeat with aFolder in filteredFolders
				...
				repeat with theNote in theNotes
					...
					if (noteLimit is not -1 and totalNotesOutput > noteLimit) or (noteLimitPerFolder is not -1 and folderNoteCount ≥ noteLimitPerFolder) then
